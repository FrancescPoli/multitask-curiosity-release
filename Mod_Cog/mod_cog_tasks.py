...
class _Reach(EnvWithAdditions):
    ...
    def __init__(self, dt=100, anti=True, rewards=None, timing=None,
        ...
        # Base timing: by default, no blank delay, unless dly=True or overridden
        self.timing = {
            'fixation': 500,
            'stimulus': 500,
            'delay': 0,
        ...
        if dly:
            # delayed variants (e.g. dlygo, dlyanti) get a non-zero delay
            self.timing['delay'] = 500

        # Explicit timing kwarg always wins
...
def dlygoseqr(**kwargs):
    env_kwargs = kwargs.copy()
    env_kwargs['anti'] = False
    env_kwargs['timing'] = {'delay': 500, 'decision': 1000}
    env_kwargs['seq'] = 1
    return _reach(**env_kwargs)
